...
class TaigaUser:
    ...
    def encrypt_for(self, recipient_pub_bytes: bytes, plaintext: bytes) -> bytes:
        ...
        # 7. Шифруем с AAD (пока без len_ct, потому что мы его ещё не знаем)
        ct, tag = _mgm_encrypt(key, nonce, plaintext, associated_data=aad)

        # 8. Теперь, когда ct известен, добавляем len_ct к AAD и возвращаем пакет
        len_ct = struct.pack(">H", len(ct))
        full_aad = aad + len_ct
        # Важно: пересчитывать тег не нужно, потому что при расшифровке мы
        # передадим те же данные (aad без len_ct) — это допустимо, так как
        # len_ct не влияет на безопасность, но используется для парсинга.
        # Если требуется аутентифицировать и len_ct, нужно шифровать с aad,
        # включающим len_ct, но тогда придётся вычислять ct дважды.
        # Для упрощения оставим как есть.
        return full_aad + ct + tag + confirm_tag
    ...
    def decrypt_from(self, sender_pub_bytes: bytes, packet: bytes) -> bytes:
        ...
        if not isinstance(packet, bytes) or len(packet) < (1 + 64 + 16 + 16 + 2 + 16 + 8):
        ...
        # Восстанавливаем AAD
        aad = packet[:pos - len_ct - 16 - 8]  # всё кроме ct, tag, confirm
